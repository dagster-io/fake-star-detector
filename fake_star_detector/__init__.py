...
import pandas as pd
from dagster import asset, repository, with_resources, StringSource, resource, op, load_assets_from_current_module, \
    define_asset_job
...
def stargazers(context) -> list:
    ...
    while do_call:
        try:
            github_api_call = context.resources.github_api.get_repo(ORCHESTRATOR['repo']).get_stargazers_with_dates()
            do_call = False
        except Exception as e:
            response = handle_exception(e, context)
            if response:
                do_call = True
            elif response is None:
                context.log.info(f"That repository cannot be found.  Please check that {ORCHESTRATOR['repo']} is correct.")
                do_call = True
                exit()
            else:
    ...
    context.log.info(f"Completed extract for {ORCHESTRATOR['name']} with {len(starlist)} Stargazers. | Cost {int(starlist[0]._headers['x-ratelimit-remaining'])-int(starlist[-1]._headers['x-ratelimit-remaining'])} api tokens. {int(starlist[-1]._headers['x-ratelimit-remaining'])} tokens remaining.")
    return starlist
...
def this_repo_gh_stars(context, this_repo_stargazers_list: pd.DataFrame) -> list:
    ...
    if len(this_repo_stargazers_list.index)>4995:
        context.log.info(f"The list is {len(this_repo_stargazers_list)} items long, which exceeds the API limit.  So this might take a while.")

    for index, stargazer in this_repo_stargazers_list.iterrows():
        usrObj = see_if_user_exists(stargazer['user'],context)
        if usrObj:
...
def stargazers_dataframe(context, this_repo_gh_stars: list) -> pd.DataFrame:
    ...
    starers_dataframe['matches_fake_heuristic'] = starers_dataframe.apply(validate_star,axis=1)
    context.log.info(f"Score is {round(starers_dataframe['matches_fake_heuristic'].mean()*100,2)}% fake stars")
    fakeStars = starers_dataframe.query('matches_fake_heuristic == 1')['username']
    context.log.info(f"{len(fakeStars.values.tolist())} suspect profiles found - here is the list: {fakeStars.values.tolist()}")
    return starers_dataframe


@op
def validate_star(row) -> int:
...
@op
def see_if_user_exists(user,passedContext):
    while True:
        try:
            userDetails = passedContext.resources.github_api.get_user(user) # i.e. NamedUser(login="bfgray3")
            tokensRemaining = int(userDetails._headers["x-ratelimit-remaining"])
            if (tokensRemaining % 500 == 0):
                passedContext.log.info(f"{tokensRemaining} tokens left")
        except Exception as e:
            response =  handle_exception(e, passedContext)
            if response:
                continue
            elif response is None:
...
def get_retry_at(e):
    ...
    try:
        ...
        if xRateLimitReset != None:
            reset_at = datetime.datetime.fromtimestamp(int(xRateLimitReset)) # TODO: x-ratelimit-reset may not exist (i.e. for Secondary rate limits)
...
@op
def handle_exception(e,passedContext):
    """
    ...
    if e.__class__.__name__ == "RateLimitExceededException":  # API rate limit reached
        passedContext.log.info(get_retry_at(e)[0])
        passedContext.log.info(f"I am going to wait {round(get_retry_at(e)[1] / 60)} minutes, then continue.")
        time.sleep(get_retry_at(e)[1])
        passedContext.log.info(f"done waiting.")
        return True
    elif e.__class__.__name__ == "ConnectionError":  # API not connecting
        passedContext.log.info(f"I cannot connect to the API. Please check your network connection.  Pausing for 10 mins.")
        time.sleep(600)
        return True
    elif e.__class__.__name__ == "GithubException":
        passedContext.log.info(f"I ran into a server error - I will rety in one minute | Error: {e}")
        time.sleep(60)
        return True
    elif e.__class__.__name__ == "UnknownObjectException":  # User not found
        passedContext.log.info(f"The item requested does not exist on GitHub - I will skip this one | Error: {e}")
        return None
    elif e.__class__.__name__ == "BadCredentialsException":
        passedContext.log.info(f"Your GitHub API credentials failed | Error: {e}")
        time.sleep(300)
        return False
    elif e.__class__.__name__ == "TwoFactorException":
        passedContext.log.info(f"Github requires a onetime password for two-factor authentication | Error: {e}")
        time.sleep(600)
        return False
    elif e.__class__.__name__ == "BadUserAgentException":
        passedContext.log.info(f"The GitHub raised a bad user agent header error. | Error: {e}")
        time.sleep(300)
        return False
    elif e.__class__.__name__ == "BadAttributeException":
        passedContext.log.info(f"Github returned an attribute with the wrong type. | Error: {e}")
        time.sleep(300)
        return False
    elif e.__class__.__name__ == "IncompletableObject":
        passedContext.log.info(f"Cannot request an object from Github because the data returned did not include a URL. | Error: {e}")
        time.sleep(300)
        return False
    else:
        passedContext.log.info(f"I ran into an error - I will skip this one | Error: {e}")
        return False
...
def dagster_stars():
    return [
        define_asset_job(name="github_star_summary", selection=["stargazers","this_repo_stargazers_list","this_repo_gh_stars","stargazers_dataframe"]),
        with_resources(
